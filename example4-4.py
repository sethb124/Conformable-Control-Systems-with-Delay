...
for axis, alpha in zip(axes, alphas):
	...
	S = np.empty((t_dis.size, 4, 4), dtype=np.longdouble)
	S[-1] = Sf
	...
	v = np.empty((t_dis.size, 4, 1), dtype=np.longdouble)
	v[-1] = vf
	...
	x = np.empty((t_dis.size, 4, 1), dtype=np.longdouble)
	x[0] = np.c_[[-0.25, -0.5, 0.25, -0.3]]
	# u is an array of real #s
	u = np.empty((t_dis.size, 3, 1), dtype=np.longdouble)
	...
	axis[0].plot(t_dis, u[:,2], label='$u_3(t)$', color='green')
